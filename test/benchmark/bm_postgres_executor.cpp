...
class PostgresFixture : public benchmark::Fixture {
 public:
  void SetUp(State &) override {
    auto &param = executor_testing::getExecutorTestParamPostgres().get();
...
BENCHMARK_F(PostgresFixture, CreateRole)(State &state) {
...
BENCHMARK_F(PostgresFixture, TransferAsset)(State &state) {
  auto result = getItf().createUserWithPerms(
      kUser,
      kDomain,
      PublicKeyHexStringView{kUserKeypair.publicKey()},
